...
// Initialize the Self Protocol verifier with MOCK mode for playground
...
const selfVerifier = new SelfBackendVerifier(
  'attps-age-assurance-playground', // scope (must match frontend QR config)
  'https://attps.social/api/playground/assure', // endpoint
  true, // mockPassport - TRUE for playground (mock passports)
...
export async function POST(request: Request) {
  try {
    ...
    // Store verification result in Netlify Blobs (separate store for playground)
    if (userContextData) {
      try {
        console.log('[Playground] Storing verification for UUID:', userContextData)
        const store = getStore('playground-verifications')
        await store.setJSON(userContextData, {
          verifiedAt: Date.now(),
          used: false,
        } as VerificationData)
        console.log('[Playground] Successfully stored verification')
      } catch (blobError) {
        console.error('[Playground] Failed to store verification in Netlify Blobs:', blobError)
    ...
    console.error('[Playground] Verification error:', error)
...
export async function GET(request: Request) {
  try {
    ...
    try {
      console.log('[Playground] Looking up verification for UUID:', uuid)
      console.log('[Playground] Self-transformed UUID:', selfTransformedUuid)
      store = getStore('playground-verifications')
      verification = await store.get(selfTransformedUuid, { type: 'json' }) as VerificationData | null
      console.log('[Playground] Verification lookup result:', verification)
    } catch (blobError) {
      console.error('[Playground] Failed to read from Netlify Blobs:', blobError)
      return Response.json(
    ...
    if (!privateKey) {
      console.error('[Playground] ATTESTATION_PRIVATE_KEY not configured')
      return Response.json(
    ...
    console.error('[Playground] Attestation signing error:', error)
    return Response.json(
      {
        success: false,
